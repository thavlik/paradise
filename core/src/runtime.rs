...
lazy_static! {
    static ref _RT: std::sync::Mutex<Option<std::sync::Weak<Runtime>>> =
...
impl PortPool {
    ...
    pub fn reserve(&self) -> std::io::Result<u16> {
        ...
        match (self.min..self.max).find(|port| !reserved.contains(port)) {
            Some(port) => {
                reserved.push(port);
                Ok(port)
            }
    ...
    pub fn release(&self, port: u16) {
        let mut reserved = self.reserved.lock().unwrap();
        match reserved.iter().position(|p| *p == port) {
...
impl Runtime {
    ...
    pub fn get() -> std::sync::Arc<Runtime> {
        unsafe {
            ...
            match &*guard {
                Some(runtime) => match runtime.upgrade() {
