...
fn basic() {
    ...
    let mut patchbays: Vec<Box<Node>> = (0..NUM_PATCHBAYS)
        ...
        .collect();
    let mut ifaces: Vec<Box<Node>> = (0..2).map(|i| Node::new(NodeKind::Interface, 8)).collect();
    let mut patchbays = patchbays
        .into_iter()
        .map(|mut pb| {
